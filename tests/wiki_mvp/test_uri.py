...
from openviking.wiki_mvp.uri import node_root_uri, sanitize_node_id, wiki_root
...
def test_resource_root_uri_keeps_exact_input():
    config = WikiMVPConfig(resource_root_uri="viking://resources/paper.md")

    assert config.resource_root_uri == "viking://resources/paper.md"


def test_wiki_root_uri_keeps_exact_input():
    config = WikiMVPConfig(wiki_root_uri="viking://wiki/my_wiki")
